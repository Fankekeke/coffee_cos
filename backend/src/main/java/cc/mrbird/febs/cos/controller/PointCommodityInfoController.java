...
public class PointCommodityInfoController {
    ...
    @GetMapping("/list")
    public R list() {
        return R.ok(pointCommodityInfoService.list(Wrappers.<PointCommodityInfo>lambdaQuery().eq(PointCommodityInfo::getDelFlag, "0")));
    }
    ...
    public R save(PointCommodityInfo pointCommodityInfo) {
        pointCommodityInfo.setCode("PCM-" + System.currentTimeMillis());
        pointCommodityInfo.setCreateDate(DateUtil.formatDateTime(new Date()));
